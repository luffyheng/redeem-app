...
// Get full order detail (including status and item_list)
...
// Get only SKUs from order
async function getOrderSKU(orderNumber, accessToken) {
  const order = await getOrderDetail(orderNumber, accessToken);
...
// Mark order as shipped (non-integrated, for virtual products)
